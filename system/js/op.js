...
var base_url = '';
var sys_url = '../system/system_process?task=';
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result == true) {
				$.ajax({
					type: 'POST',
					url: 'system_process?task=logout',
					data: { 'rtype': 'AJAX' },
					success: function (data) {
						console.log(data);
						var obj = JSON.parse(data);
...
$(".cancelreceipt").on('click', function () {
	...
	if (remarks == '' || remarks === null) {
		...
		$.ajax({
			method: 'post',
			url: 'system/master_process.php?task=cancel_receipt',
			data: {
				receipt_id: id,
...
$(".cancel_acctxn").on('click', function () {
	...
	if (remarks == '' || remarks === null) {
		...
		$.ajax({
			method: 'post',
			url: 'system/master_process.php?task=cancel_acctxn',
			data: {
				txn_id: id,
				cancel_remarks: remarks
			},
...
$(document).on('change',".upload_multi_img",function(){
    ...
    // console.log(formData);
     ...
     beforeSend:function()
     {
          $("#attachment_display").html("<h1 id='waiting' class='text-center text-muted'> <i class='fa fa-spinner fa-spin' style='font-size:40px'></i> <br> Please Wait While Uploading the files.. </h1>");
     },
     complete:function()
     {
          $("#waiting").hide();
...
$(document).ready(function () {
    $(".summernote").each(function () {
        $(this).summernote({
            ...
            callbacks: {
                onBlur: function (e) {
                    // Get the value from the summernote editor
                    var data = $(this).summernote('code');
                    console.log(data);
                    // Find the corresponding hidden input and set its value
                    $(this).closest('.form-group').find(".summerdata").val(btoa(data)); // btoa() encodes to Base64
                }
            }
        });
    });
...
// user Select2 for drop down any where
$(".select2").select2();
