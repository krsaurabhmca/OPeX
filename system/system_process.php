	...
	switch ($task) {
		...
		case "reset_opex":
			...
			foreach((array)$tres['data'] as $table)
			{
				...
				$res[] = delete_data('op_master_table',array('table_name'=>$table_name));
				$res[] = delete_data('op_menu',array('table_id'=>$id));
				$res[] = delete_data('op_role',array('table_id'=>$id));
		...
		case "delete_table":
			extract($_REQUEST);
			$table_name  = get_data('op_table',$id,'table_id')['data'];
			delete_data('op_master_table',array('table_name'=>$table_name));
			delete_data('op_menu',array('table_id'=>$id));
			delete_data('op_role',array('table_id'=>$id));
  ...
  <p style="text-align: center;"><a href="'.$base_url.'" style="display: inline-block; padding: 10px 20px; background-color: #007bff; color: #ffffff; text-decoration: none; border-radius: 5px;">Login Now</a></p>
	...
	case 'sync_table':
		...
		case "verify_login": // Delete Any Data From Table 
			...
			$res = get_all('op_user', '*', array('user_name' => $user_name, 'user_pass' => md5($user_pass),'user_status'=>'ACTIVE'));
			...
			if ($res['count'] == 1) {
				...
				if( trim(get_data('op_user',remove_space($user_name), 'allow_otp','user_name')['data']) =='YES')
				{
					...
					$sms ="Thanks for interest in $inst_name Your App Login OTP is $otp Regards $inst_name OFFAPP";
					send_msg($mobile,$sms, "1207166936475956518");
					send_mail($email,"Your Login Otp is $otp", $sms);
